...
export default function AdminJobManagementPage() {
    ...
    const columns = [
        ...
        {key: 'actions', header: 'Actions', render: (job) => <div className={styles.actions}><Link className={styles.detailLink} to={`/jobs/${getJobId(job)}`}>View</Link><Link className={styles.detailLink} to={`/admin/jobs/${getJobId(job)}/applications`}>Applications</Link><Button variant="secondary" onClick={() => runAction(job, job.published ? 'unpublish' : 'publish')} disabled={busyId === getJobId(job)}>{job.published ? 'Unpublish' : 'Publish'}</Button><Button variant="danger" onClick={() => runAction(job, 'soft-delete')} disabled={busyId === getJobId(job)}>Soft-delete</Button></div>},
    ];
